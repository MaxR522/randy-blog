...
2. One brand with the admin: same neutrals, same Inter, same buttons and inputs. The public side adds one accent, one serif and a magazine-like layout.
3. No gradients, at most one shadow (sticky navbar once scrolled). Thin rules (1 px grey-light, 2 px primary) do the structuring.
4. The accent (Bleu encre) is rare: links, category labels, active states, ranking numbers, focus ring, the Encadré, the subscription block. Buttons stay monochrome.
...
| Accent | `accent-50 … 900`. 600 `#365AC0` for links, focus, numbers (6.2:1 on white). 700 for labels on tint and hover. 50 for Encadré and subscription background, 200 for their hairline | `text-accent-600`, `bg-accent-50`, `border-accent-200` |
...
| Fonts | `--font-sans` Inter, `--font-serif` Newsreader | `font-sans`, `font-serif` |
...
| Focus | 2 px `accent-600`, offset 2 px, on `:focus-visible` (set in the base layer, never remove) | |
...
Set the family with `font-serif` or `font-sans`; the size token sets size, line height, letter spacing and weight together. Mobile / desktop (laptop and up) values:

| Style | Classes | Mobile | Desktop | Notes |
|---|---|---|---|---|
| Display | `font-serif text-display` | 44 / 1.02 | 72 / 1.0 | home hero title. Tablet 56 |
| H1 | `font-serif text-h1` | 34 / 1.12 | 52 / 1.08 | article title, page titles. Tablet 40 |
| H2 | `font-serif text-h2` | 26 / 1.2 | 32 / 1.2 | article body h2 |
| Section title | `font-serif text-section` | 26 | 30 | section headers |
| H3 | `font-serif text-h3` | 21 | 24 | |
| H4 | `font-sans text-h4` | 17 | 17 | Inter 600 |
| Card hero / large | `font-serif text-card-hero`, `text-card-large` | 30 / 28 | 40 / 34 | |
| Card | `font-serif text-card` | 20 | 21 | `text-card-wide` (24) when the card is wider than about 380 px |
| Card compact | `font-serif text-card-compact` | 17 | 17 | |
| Chapô | `font-serif text-chapo text-grey-dark` | 19 / 1.45 | 22 / 1.45 | weight 400 |
| Article body | `.article-body` | 18 / 1.65 | 20 / 1.7 | Newsreader 400, column 608 px (about 68 characters) |
...
Article body is serif on purpose (personal opinion column); Inter is kept for everything that is not prose (tables, captions, buttons).
...
### Input
Height 48–52, `rounded-md border border-grey-medium px-4 text-body`, focus = base-layer ring plus `border-primary`. Error: `border-red-600`, inner 1 px red-600 ring, message below with icon (`text-red-700 text-body-small font-medium`, `role="alert"`, `aria-describedby`).
...
### SectionHeader
`border-t-2 border-primary pt-3 mb-6 flex min-h-11 items-center justify-between gap-4`; title `font-serif text-section`; optional link « Voir plus » `inline-flex min-h-11 items-center gap-1.5 text-body-small font-medium text-accent-600 underline underline-offset-4` with a 16 px arrow-right.

### Navbar (sticky)
Desktop: 68 px, `border-b border-grey-light`, three-column grid `grid-cols-[1fr_auto_1fr]`. Left: contextual link (`A propos de ma pomme` on the home page → author page; `Revenir à la page d'accueil` with arrow-left elsewhere → home). Centre: wordmark « Randy Donny » in `font-serif` 28 px 600. Right: primary button « S'abonner ». `shadow-navbar` only once scrolled.
Mobile: sticky bar 56 px (wordmark left, « S'abonner » right). The contextual link is a second row (44 px, bottom border) that scrolls away.
...
### Footer
`border-t-2 border-primary`. Desktop: three columns (`1.3fr 1fr 1fr`): wordmark + tagline in serif italic, « Suivre » (Facebook, X, LinkedIn, YouTube, each 44 px tall), « Informations » (« Politique de confidentialité »). Bottom row after a hairline: « © 2026 Randy Donny » left, developer credit right (`Site conçu et développé par [Nom du développeur]`, a placeholder). Mobile: stacked, socials in two columns.

### SearchBar
Large (home hero): 64 px (56 mobile), `border border-primary rounded-md`, leading search icon, input 18 px, attached primary « Rechercher » button on desktop; on mobile the button is a 56 px square icon button with `aria-label`. Compact (results): 48 px, icon leading, no button. Placeholder « Rechercher un article… ». Input has a visually hidden `<label>`.

### Cards (one visual language: 3:2 image, serif title, Inter meta)
Every image: `aspect-card w-full rounded-sm object-cover` (always set the ratio, never rely on intrinsic size). Image links are `tabindex="-1" aria-hidden`; the title is the real link.

| Card | Use | Content and sizes |
|---|---|---|
| Featured (hero) | one per page, home hero right column | « À la une » overline with a 2 px top rule, image, chips, title `text-card-hero`, chapô 20 (18 mobile), date |
| Large | lead of a group | image, chips, title `text-card-large`, chapô 18 (17), date |
| Standard | grids and rows | image, title `text-card` (`text-card-wide` in 3-column grids), date |
| Compact | dense lists, all mobile lists | 96 × 64 thumbnail left, title `text-card-compact`, date; hairline between items |
| Numbered | « Les plus lus » only | no image; number `font-serif` 56 (44 mobile) `text-accent-600`, title 20, date; render as `<ol>`, number is `aria-hidden` |
...
<article className="min-w-0">
  ...
  <h3 className="mt-3.5 font-serif text-card text-primary text-balance"><a href={url}>{title}</a></h3>
  <p className="mt-2 text-meta font-medium text-grey-dark">{date}</p>
</article>
```
Spacing: image → title 14, title → date 8, hero/large: image → chips 20/16, chips → title 12–14, title → chapô 10–12, chapô → date 14–16.
...
- Empty state: centred, search icon (`text-grey-medium`), serif 30–34 « Aucun article ne correspond à votre recherche. », `role="status"`.
...
Full-width band, `bg-accent-50 border-y border-accent-200`, padding 80 (48 mobile). Desktop: two columns (gap 64), left « Restez informé » (`font-serif` 48 / mobile 34) + « Recevez mes nouveaux articles directement dans votre boîte mail. »; right: label « Adresse email », input + primary « S'abonner » (52 px, stacked on mobile), consent notice (`text-caption text-grey-dark`, link in `text-accent-700 underline`). States and exact messages:
...
- **Previous / next**: two cells side by side (divider between), stacked on mobile; overline « Article précédent » (arrow-left) / « Article suivant » (arrow-right) in accent-700, serif 26 (21) title, date. Each cell is one link.
...
1. **Home** (`Accueil*.dc.html`): navbar (home variant) → hero (`laptop:grid-cols-2 gap-16 items-center`; text first when stacked) with display title « Je pense, donc j'essuie… », description with links (Hautetfort, Facebook, X, LinkedIn, chaîne YouTube), large search bar, featured card → « Dernières publications » (desktop `laptop:grid-cols-2`: large card left, 2 × 2 standard cards right; below laptop: large card then 4 compact items) → « Les plus lus » (`laptop:grid-cols-5` numbered items; list on mobile) → one section per category, e.g. Politique, Culture, Société (`tablet:grid-cols-2 laptop:grid-cols-4` standard cards; on mobile a horizontal scroller with 248 px snap cards bleeding to the screen edge) → « Tous les articles » (`tablet:grid-cols-2 laptop:grid-cols-3`, row gap 48; compact list on mobile; « Voir plus ») → SubscriptionBlock → footer.
...
3. **Author** (`Auteur*.dc.html`): avatar (288 px circle, 160 mobile) with « Crédit photo : … », display name `font-serif` 80 (44), rich-text biography in the article prose style, four outlined social buttons, « Derniers articles » with 3 standard cards.
4. **Search results** (`Recherche*.dc.html`): h1 « Résultats pour « démocratie » », compact search bar pre-filled, count, list (cover 240 px, chips, serif 28 title, excerpt with `<mark>` highlights `bg-accent-100`, date · reading time), pagination. Column 880 px. Empty variant `Recherche-vide`.
...
7. **Newsletter email** (`Newsletter*.dc.html`): 600 px, table layout, inline styles, single column, `Newsreader, Georgia, serif` (Georgia is the real fallback in most clients), wordmark header, free text, article card (cover, chips, title, chapô, « Lire l'article » button as a padded `<a>` in a `<td>`), footer with social text links, « Vous recevez cet email car vous êtes abonné à la newsletter de Randy Donny. » and « Se désabonner ». Use the same hex values from the theme (emails cannot use CSS variables).

## 7. Implementation notes

- Fonts: self-hosted by `laravel-vite-plugin/fonts` (`bunny()` in `vite.config.ts`, emitted by `@fonts` in `resources/views/app.blade.php`): Inter 400/500/600/700 and Newsreader 400/600 + italic, `display=swap`, roman files preloaded, fallbacks Georgia and system-ui.
